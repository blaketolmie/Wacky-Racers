...
\item
  Keep the crystal close to the MCU and away from high-speed traces.
\item
  Place switches/jumpers so they can be easily accessed.
\item
...
\item Keep planes away from the radio antenna; otherwise the radio
  range is limited.  Also keep high-speed traces away from the radio
  antenna.
...
\begin{enumerate}
\item Use normal vias but not blind or buried vias\footnote{Blind and
  buried vias are much harder to make and thus the PCB is more
  expensive.}.  The smallest via that can be made for this assignment
  has a 0.3\,mm hole with a 0.8\,mm annulus.  However, it is better to
  use a via with a 0.5\,mm hole and 1.0\,mm annulus.

\item There is no need to scrimp on vias.  Use vias to ground and
  power planes rather than using traces.

\item For a prototype PCB it is best to use untented vias since these
  are useful for test points and rework.  Tented vias have a layer of
  insulating solder resist.  These are necessary when placing a via
  underneath a component with a metal heatsink pad.
\end{enumerate}
