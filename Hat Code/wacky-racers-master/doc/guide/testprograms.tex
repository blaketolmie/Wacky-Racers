...
\section{LED}
\label{led-interfacing}

Here's an example of a program to flash an LED using a paced loop.

\inputminted{C}{../../src/test-apps/ledflash1/ledflash1.c}

The frequency of the slowest paced loop is given by:
%
\begin{displaymath}
  \frac{f_{\mathrm{CPU}}}{2^{24}}
\end{displaymath}
%
where $f_{\mathrm{CPU}}$ is the CPU clock frequency.  For a 96\,MHz
clock, the slowest paced loop is about 6\,Hz.  Note, the paced loop
implementation is not energy efficient since it does busy-wait polling
of a hardware timer/counter.

Here's another implementation:

\inputminted{C}{../../src/test-apps/ledflash2/ledflash2.c}

This sets up a hardware timer/counter to generic a periodic interrupt
at 1\,kHz.  The interrupt service routine calls the function
\code{heartbeat} to flash the LED.  There is nothing for the main loop
to do, so it executes the WFI (wait for interrupt) instruction.  This
disables the CPU clock (but not the timer/counter clock) to save
power.  Note, there are no shared variables between the \code{main}
and \code{heartbeat} functions so there are no race conditions.
...
To get this program to work you need to compile it and program the SAM4S (see
Section~\ref{programming}~\protect\hyperref[programming]{Programming}).
...
On the Windows machines in the ESL you can use Tera Term or the VS Code Serial
Monitor extension. On opening the application, select the correct \code{COM}
port from the \code{Serial} dropdown. If there are multiple \code{COM} ports
listed and you can't tell which one belongs to the USB serial device, unplug the
USB and observe which \code{COM} port number is added to the dropdown menu when
you reinsert it.
...
\begin{description}
  \item[Tera Term (Windows)] Open the \verb|Setup > Terminal...|
  ...
  \item[VS Code Serial Monitor (all OSes)] Install the Microsoft \textit{Serial
  Monitor} extension, ensure the \code{Line Ending} dropdown is set to \code{LF}
  and click \code{Start Monitoring}. You can send data to the device by entering
  it in the text box at the bottom and clicking the \code{Send Ctrl-C} button.

  \item[gtkterm (Linux)] In the \verb|Configuration| dropdown menu, make sure
