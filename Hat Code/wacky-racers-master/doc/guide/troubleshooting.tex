...
If the hardware does not work after erasing flash:
%
\begin{enumerate}

  \item Check that the DDP pin is pulled to 3.3\,V.  If not, check the
  DDP pin is soldered correctly.
...
If the USB does not work with your program:

\begin{enumerate}

  \item Check that the DDP pin is pulled to 3.3\,V.  If not, check
    that the \code{USB_VBUS_PIO} macro, defined in \file{target.h}, is
    set to the correct PIO pin.  Note, this macro only needs to be
    defined if you need to remove and re-install the USB cable while
    the SAM4S is running.  I recommend commenting it out.
...
If the USB serial connection seems to work some of the time, it could
be because you have plugged in the USB connection after powering
your device. You should have the USB physically connected before
applying power to your device (through the ST-link or battery power
conenctor) for the program to initialise correctly.

 \section{Buck converter}
\label{buck-converter-testing}

If the 5\,V rail is shorted to ground, check:
%^
\begin{enumerate}
\item The flyback diode is installed the corret way
\item There are no shorts across the decoupling capacitors due to too
  much solder
\end{enumerate}
...
\item \emph{The MCU runs but gets very hot}.  Check the 3.3\,V and
  1.2\,V power supplies to the MCU.  Chances are that you are running
  the MCU from 5\,V!.

\item \emph{The MCU runs but gets hot}.  Check that you have not
  configured a PIO pin as a high output that is shorted to ground or
  configured a PIO pin as a low output shorted to 3.3\,V.  Note, this
  can occur with a misconfigured PIO connected to a switch without a
