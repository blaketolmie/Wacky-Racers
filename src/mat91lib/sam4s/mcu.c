...
mcu_xtal_mainck_start (void)
{
    ...
    /* Wait for switchover to XTAL oscillator */
    while (! (PMC->PMC_SR & PMC_SR_MOSCSELS))
        continue;

    /* Wait for main clock frequency to be measured */
...
mcu_clock_init (void)
{
    /* To minimize the power required to start up the system, the main
       ...
       See datasheet Section 29.14 'Programming sequence' (p. 522).
    */
    ...
    /* Select MAINCK for MCK. This should already be selected on hardware
       reset but is not guaranteed to be selected on software reset
       if the chip was reset after selecting the PLL as the clock
       source. */
    if (mcu_mck_css_get () != PMC_MCKR_CSS_MAIN_CLK)
    {
        PMC->PMC_MCKR = (PMC->PMC_MCKR & ~PMC_MCKR_CSS_Msk)
            | (PMC_MCKR_CSS_MAIN_CLK << PMC_MCKR_CSS_Pos);
    ...
    /* Check main clock frequency is non-zero. TODO: should check the
       value matches the expected frequency. Also if 0, could switch
       back to fast RC oscillator. See p. 522 of datasheet. */
    if (CKGR_MCFR_MAINF(PMC->CKGR_MCFR) == 0)
        mcu_clock_error(0);
#endif
    ...
    /* Set prescaler. Needs to be written before programming CSS
       with MCKRDY checks in between (see p. 523 in datasheet) */
    PMC->PMC_MCKR = (PMC->PMC_MCKR & ~PMC_MCKR_PRES_Msk)
        | (MCU_MCK_PRESCALER_VALUE << PMC_MCKR_PRES_Pos);

    mcu_mck_ready_wait (2);

    /* Switch to PLLA_CLK for MCK.
       TODO: do we need to change PLLA divisor (PLLADIV2)? */
    PMC->PMC_MCKR = (PMC->PMC_MCKR & ~PMC_MCKR_CSS_Msk)
        | (PMC_MCKR_CSS_PLLA_CLK << PMC_MCKR_CSS_Pos);

    mcu_mck_ready_wait (3);

    /* Check the register was written correctly. */
