...
\title{Wacky Racers 2026 Instructions}
\author{Michael Hayes}
\date{Version 1.1111, \today}
...
\item Use an ARM microcontroller (Microchip SAM4S).
...
\begin{flushleft}
  \begin{tabular}{ c l l }
    ...
    \hline
    T2-1 & General          &                   \\
    T2-2 & General          & Blinky milestone            \\
    T2-3 & General          & Accelerometer/motors milestone        \\
    T2-4 & General          & Radio control milestone     \\
    T2-5 & General          & Functionality assessment     \\
    T2-6 &                  & Race, critique  \\
  \end{tabular}
\end{flushleft}
...
\item Do not underestimate the blinky milestone.  The program is
  ...
  a functional PCB and a functional toolchain.
...
\item [PCB submission] Submit your PCB design to Learn for
  ...
  It is not possible for me to cover all aspects of PCB design in
  lectures before your PCB submission.  I recommend you read the
  lecture notes for power supply decoupling and crosstalk.
...
\item [Blinky] Demonstrate that you can blink an LED controlled by the SAM4S.
...
\item [Accelerometer/motors]
  ...
  \item Map the accelerometer readings to two duty cycles corresponding to the \textbf{two racer locomotion motors}, providing full range of control (variable speed and steering control in the forward and backward directions), and output the result to a PC using USB CDC.
  \end{itemize}

  For the Wacky Racer,
  \begin{itemize}
    \item Demonstrate the operation of the locomotion motors using PWM signals (as provided in the test apps).
    \item Provide full range of control for the locomotion motors (variable speed and steering control in the forward and backward directions) based on duty cycle values entered on a PC using USB CDC.
    \end{itemize}

\item[Radio control]

  Demonstrate sending commands from the Wacky Hat to the Wacky Racer
  and vice-versa over a radio link. This includes the racer being controllable
  by hat board orientation and the hat board visibly/audibly reacting to the
  racer bumper being hit.
...
The software milestones must be demonstrated to a T.A. by the end of
the lab for your allotted stream.  If you need an exception to this,
see \fredy\ with a \emph{very} good reason (such as isolating with
COVID).
...
Sleep mode is where you shut down the MCU to save energy (say using a
pushbutton press) and use a pushbutton to wake the MCU via a wakeup
interrupt.  It is easy to get this wrong and hard to save every
microamp.
...
\item [Week~1]\mbox{}\\[-0.4cm]
  ...
  \item Attempt Altium schematic tutorial.  The tutorial is on Learn.
...
\item [Week~4]\mbox{}\\[-0.4cm]
  ...
  \begin{itemize}
  \item Attempt Altium PCB tutorial.  The tutorial is on Learn.
...
\item [Break]\mbox{}\\[-0.4cm]
  ...
  \begin{itemize}
  \item Watch SMT lab induction video on Learn.

  \item Book slot to use SMT lab.

  \item Populate PCB in SMT lab.

  \item Perform initial tests.
  \end{itemize}

\item [Week-7]\mbox{}\\[-0.4cm]

  \begin{itemize}
  \item Write blinky program.  Do not underestimate the effort needed
    to get a toolchain working if you don't use the ECE computers.
  \end{itemize}
...
\item [Week~11]\mbox{}\\[-0.4cm]
  ...
  \begin{itemize}
  \item Demonstrate full functionality, see \refsec{functionality}.
...
\item [Week~12]\mbox{}\\[-0.4cm]
  ...
  \item Return chassis, batteries, and ST-link programmers to Diego.
...
\item \emph{Why use the SAM4s MCU?}  For this application most MCUs
  ...
  before.  This is an ARM based MCU made by Atmel (but now owned by
  Microchip) I have used this in a number of projects.  Indeed we used
  to teach this chip in ENCE361.  There are many other similar MCUs
  made by different manufacturers such as the STM32 that would just as
  suitable.
...
You can use AI for assistance but your mileage will vary\footnote{I
  have seen dubious circuits generated.}.  If you do, please
acknowledge in your critique.
